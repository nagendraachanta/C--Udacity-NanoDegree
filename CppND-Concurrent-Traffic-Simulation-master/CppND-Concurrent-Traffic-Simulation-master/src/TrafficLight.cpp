...
void TrafficLight::cycleThroughPhases()
{
    ...
    std::mt19937 eng(randd());
    std::uniform_int_distribution<>rng(4,6);
    double looptime=rng(eng);
    std::chrono::time_point<std::chrono::system_clock>initialtime=std::chrono::system_clock::now();
    while(true){
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
        long timebetloops=std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now()-initialtime).count();
        if(timebetloops>=looptime){
